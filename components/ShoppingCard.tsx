...
function ShoppingCard() {
  ...
  const handleIncreaseData = (quantity: number, id: number) => {
    const count = quantity + 1;
    mutate({id, count}); // Call the updateProductQuantity function with the productId and updated quantity
  };

  const handleDecreaseData = (quantity: number, id: number) => {
    if (quantity > 1) {
      const count = quantity - 1;
      mutate({id, count}); // Call the updateProductQuantity function with the productId and updated quantity
    } else if (quantity == 1) {
                ...
                <div className='border-l-black-items border-opacity-10  md:flex md:flex-col md:gap-y-3 md:border-l md:py-4 md:pl-3'>
                  ...
                  <div className='hidden items-center justify-between md:mt-4 md:flex md:justify-center'>
                    <div className='flex items-center justify-center gap-2'>
                      <div onClick={() => handleDecrease(item.count, item.id)}>
                        <MiddleIcon>
                          <MinusIcon />
                        </MiddleIcon>
                      </div>
                      <div className='numberItemBg flex h-9 w-[74px] items-center justify-center '>{item.count}</div>
                      <div onClick={() => handleIncreaseData(item.count, item.id)}>
                        <MiddleIcon>
                          <PlusIcon />
                        </MiddleIcon>
                      </div>
